...
public class StartCheckActivity extends BaseCommitFootViewActivity {
    ...
    private CheckDetailBean.DataBean savedCheckBean;

    @Override
    public void initData() {
        HawkProperty.UPLOAD_TYPE = 1;
        unSavedLogic();
        savedCheckBean = Hawk.get(HawkProperty.START_CHECK + unitBean.getId());
    ...
    private void unSavedLogic() {
        ...
        if (getIntent() != null) {
            ...
            adapter.setNewData(mPresenter.getCheckData(null,true));
            if (unitBean != null) {
    ...
    protected void saveDraft() {
        ...
        if (adapterDataBean != null) {
            CheckDetailBean.DataBean savedCheckBean = adapterDataBean.getCheckDetailBean();
            savedCheckBean.setQualified(mRadioQualifiedRb.isChecked()?1:2);
            Hawk.put(HawkProperty.START_CHECK + unitBean.getId(), savedCheckBean);
            ...
            public void onCheckedChanged(RadioGroup group, int checkedId) {
                switch (checkedId) {
                    ...
                    case R.id.radio_unqualified_rb:
                        //不合格
                        adapter.setNewData(mPresenter.getCheckData(savedCheckBean,false));
